<script lang="ts">
    ...
    import type { CreateTagLoad,CreateTagRes,SearchTagLoad,SearchTagRes } from "$Types/api";
    ...
    async function handleSearch() {
        const apiLoadData:SearchTagLoad = { query:value }
        const apiResponse:SearchTagRes = await postJson(`/routes/${routeData.ID}/tags/search`,apiLoadData)
    ...
    async function createItem(e:any) {
        ...
        const apiResponse:CreateTagRes = await postJson(`/routes/${routeData.ID}/tags/create`,apiLoadData)
        // If tag was created
