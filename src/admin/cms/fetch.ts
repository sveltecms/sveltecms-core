...
export default class Fetch {
    ...
    async users(props:FetchUsersLoad){
        ...
        // Add skip objects
        if(props.pageNumber) usersCursor.skip(props.pageNumber*svelteCMS.config.assetsPerPage-1)
        const userDbResult:any = usersCursor.map((data:any)=>{ data['_id']=data['_id'].toString();return data}).toArray()
        const user:Promise<UserData|null> = userDbResult
        return user
    }
}