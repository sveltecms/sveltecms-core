<script lang="ts">
    ...
    export let elementData:RouteValueData['elements'][0] = {ID: "",name: "", type: "input",value: "string" }
    // Types
    ...
    function handleElementUpdate(){
        checkError = true
        /** Validate element */
        const validated = elementData.ID.trim().length===0 ? "Element ID: can not be empty" : elementData.name.trim().length===0 ? "Element name: can not be empty" : true
        if(validated!==true){
            newToast({type:"error",msg:validated}) ; return
        }
    ...
    const elementTypes = [ "textArea","input","inputNumber","slug","content","image" ]
</script>

<div class="elementEditor" class:error>
    <Label text="Add Element" btnText={showElementEditor?"Close":"Add"} on:click={()=>showElementEditor=!showElementEditor}/>
...
<style>
    ...
    .elementEditor.error{
        background-color: rgb(196 114 114 / 7%);
        padding: 5px;
        border-radius: 5px;
    }
</style>