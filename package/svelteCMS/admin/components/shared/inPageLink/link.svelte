<script lang="ts">
    ...
    $: isARoute = $page.url.pathname.includes("/admin/routes/") && ( $page.url.pathname.endsWith(`${routeID}/new-object`) || $page.url.pathname.endsWith(`${routeID}/edit`))
    $: active = link.href===$page.url.pathname ? true : (isARoute && link.href.includes(`/admin/routes/${routeID}`))
</script>

<a data-sveltekit-prefetch href={link.href} class="link" class:active>
    {link.title}
</a>
